...
`image:`/`services:` do not.

`docker run` and Testcontainers work with *Docker in jobs*: a service `docker` in the
stack speaks the Docker API and starts every container as a container of the stack
([docs/docker-api.md](docs/docker-api.md)). Containers have no stdin, no TTY and no
privileges.
