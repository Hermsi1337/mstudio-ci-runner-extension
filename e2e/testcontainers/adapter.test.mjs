...
describe("testcontainers on mittwald", { timeout: 600_000 }, () => {
    ...
    it("starts PostgreSQL and waits for its health check", async () => {
        pg = await new PostgreSqlContainer("postgres:16-alpine")
            ...
            .start();
        const result = await pg.exec(["psql", "-U", pg.getUsername(), "-d", pg.getDatabase(), "-tAc", "select 6*7"]);
    ...
    it("reaches a container through its mapped port", async () => {
        web = await new GenericContainer("python:3-alpine")
            .withCommand(["sh", "-c", "cd /srv && exec python3 -u -m http.server 8000"])
            .withExposedPorts(8000)
            .withNetwork(network)
            .withNetworkAliases("web")
            .withCopyContentToContainer([{ content: "hello from copy", target: "/srv/x.txt" }])
            .withWaitStrategy(Wait.forLogMessage(/Serving HTTP/))
            .start();
        const res = await fetch(`http://${web.getHost()}:${web.getMappedPort(8000)}/x.txt`);
    ...
    it("reports exec output per stream and the exit code", async () => {
        const result = await web.exec(["sh", "-c", "echo out; echo err >&2; exit 4"]);
    ...
    it("copies files into a running container and back", async () => {
        await web.copyContentToContainer([{ content: "late copy", target: "/srv/late.txt" }]);
    ...
    it("waits for listening ports by default", async () => {
        const nginx = await new GenericContainer("nginx:alpine").withExposedPorts(80).start();
        const res = await fetch(`http://${nginx.getHost()}:${nginx.getMappedPort(80)}/`);
        assert.equal(res.status, 200);
        await nginx.stop();
    });
});
