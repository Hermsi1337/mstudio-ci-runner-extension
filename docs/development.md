...
(`0001_runner_tags.sql`) and change the `tag` in `meta/_journal.json` to match. Never
edit an applied migration; a fresh database gets `0000_initial.sql`.
...
| `extension:sync` | Write marketplace texts, logo and fragment properties from `deploy/mstudio/extension.yaml` into mStudio ([mstudio-setup.md](mstudio-setup.md#marketplace-entry-and-frontend-fragment)) |
...
| `runner:build` | Build all runner images and the builder image locally via `scripts/build-runner-images.sh` ([runner-image.md](runner-image.md), [image-builds.md](image-builds.md)) |
