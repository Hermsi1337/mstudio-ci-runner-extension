...
| `image-build.test.ts` | `docker build --push` from the runner through the builder into a registry, image id and metadata file match the pushed digest, a failing build keeps its exit code, `apt-get install procps` on a Debian base image passes (Alpine files like `/etc/sysctl.conf` must not collide with conffiles), a base image with file capabilities builds with a warning, BuildKit only features and a foreign platform in `FROM` (arm64 on an amd64 host, amd64 on an arm64 host) are refused before a job is queued ([image-builds.md](image-builds.md)). Every test starts its own builder without `CAP_SETFCAP`, like on Container Hosting, because kaniko destroys the container it builds in |
